...
public class OrderRepository {
    ...
    public Integer add(OrderSaveDto dto) {
        var repo1 = new SupplierRepository();
        var d = dto.getDate();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        LocalDate date = LocalDate.parse(d, formatter);
        var supplier = (Supplier)repo1.getSupplier(dto.getSupplierId());
        var order = new Order(0, LocalDate.now(), supplier, dto.getPartNo(), dto.getQuantity(), date);
        return add(order);
    }
    ...
    public void delete(int id){
        ...
        try{
            tx = session.beginTransaction();
            var s = session.get(Order.class, id);
